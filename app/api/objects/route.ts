...
export async function POST(req: NextRequest) {
  ...
  if (!Array.isArray(connections)) {
    return NextResponse.json({
      success: false,
      error: "Formato inválido. Esperado um array de conexões.",
    });
  }
  ...
  for (const connection of connections) {
    try {
      ...
      if (client === "sqlite3" && !connection.file_path) {
        throw new Error(
          "O caminho do arquivo SQLite (file_path) está ausente."
        );
      }
      ...
      // === SQLite ===
        ...
        const enrichedTables = await Promise.all(
          tables.map(async (t) => {
            ...
            const pragma = Array.isArray(pragmaRes[0])
              ? pragmaRes[0]
              : pragmaRes;
      ...
      results[connection?.id || "desconhecido"] = {
